      ...
      <div className="space-y-5">
        <section className="p-5" style={glass}>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            />
            <button type="button" className="scholar-btn" onClick={() => validateAndSaveMutation.mutate()}>
        ...
        <section className="p-5" style={glass}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label>
              ...
              <select value={form.llm_model ?? ''} onChange={(event) => setForm((current) => ({ ...current, llm_model: event.target.value }))} className="w-full px-3 py-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', color: 'var(--text)' }}>
            ...
            <label>
              ...
              <input type="number" min={1} max={100} value={form.daily_new_cards_limit ?? 20} onChange={(event) => setForm((current) => ({ ...current, daily_new_cards_limit: Number(event.target.value) || 20 }))} className="w-full px-3 py-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', color: 'var(--text)' }} />
            </label>

            <label>
              <span className="block mb-2" style={{ color: 'var(--text-secondary)', fontSize: '0.88rem' }}>Quiz questions per run</span>
              <input type="number" min={1} max={20} value={form.questions_per_document ?? 10} onChange={(event) => setForm((current) => ({ ...current, questions_per_document: Number(event.target.value) || 10 }))} className="w-full px-3 py-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', color: 'var(--text)' }} />
            </label>

            <label>
              <span className="block mb-2" style={{ color: 'var(--text-secondary)', fontSize: '0.88rem' }}>Desired retention</span>
              <input type="number" min={0.5} max={0.99} step={0.01} value={form.desired_retention ?? 0.9} onChange={(event) => setForm((current) => ({ ...current, desired_retention: Number(event.target.value) || 0.9 }))} className="w-full px-3 py-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', color: 'var(--text)' }} />
            </label>
          </div>

          <div className="flex justify-end mt-5">
            <button type="button" className="scholar-btn" onClick={() => saveMutation.mutate(form)}>
